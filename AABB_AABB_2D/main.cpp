...
void noncontinuousCollisionUpdate(float dt)
{
	...
	//Move the AABBs
	AABB1->center = glm::vec2(box1Body->position);
	AABB2->center = glm::vec2(box2Body->position);
...
void main()
{
	...
	//Create window
	window = glfwCreateWindow(800, 800, "AABB - AABB (2D Dynamic Collision Detection)", nullptr, nullptr);
	...
	//box1 creation
	box1 = new struct Mesh(6, boxVerts, GL_TRIANGLES);
	//Alter mesh for box2
	for (int i = 0; i < 6; i++)
	{
		boxVerts[i].g = 0.0f;
	...
	//box2 creation
	box2 = new struct Mesh(6, boxVerts, GL_TRIANGLES);
	...
	AABB1 = new AABB(glm::vec2(box1Body->position), boxScale * glm::vec2(2.0f));
	AABB2 = new AABB(glm::vec2(box2Body->position), boxScale * glm::vec2(2.0f));

	//Print controls
	std::cout << "Controls:\nPress and hold spacebar to disable continuous collision detection. When two boxes collide, continue the simulation by toggling continuous collision detection on and off.\n";
